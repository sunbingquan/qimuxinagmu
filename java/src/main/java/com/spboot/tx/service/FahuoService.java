...
public class FahuoService {
    ...
    @Resource
    private FahuoMapper mapper;

    public ResponseData<Fahuo> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Fahuo>> selectAll(Wrapper<Fahuo> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Fahuo>> selectAll() {
        QueryWrapper<Fahuo> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Fahuo> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Fahuo> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Fahuo> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("dingdanid")) && Convert.toInt(map.get("dingdanid")) > 0) {
            // 大于0 则写入条件
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Fahuo>> filter(Map<String, Object> map) {
        Class<Fahuo> aClass = Fahuo.class;
        com.jntoo.db.QueryWrapper<Fahuo> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Fahuo> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Fahuo entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("update dingdan set zhuangtai='已发货' where id='" + post.get("dingdanid") + "'");

            DB.execute(
                "INSERT INTO fahuoshangpin(fahuoid,jiajuid,bianhao,mingcheng,fenlei,pinpai,tupian,jiage,shuliang,xiaoji,goumairen) SELECT '" +
                charuid +
                "',jiajuid,bianhao,mingcheng,fenlei,pinpai,tupian,jiage,shuliang,xiaoji,goumairen FROM dingdanshangpin WHERE dingdanid='" +
                post.get("dingdanid") +
                "'"
            );
    ...
    public ResponseData<Object> update(Fahuo entityData, Map post) {
