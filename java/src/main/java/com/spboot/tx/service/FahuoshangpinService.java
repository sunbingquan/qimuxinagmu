...
@Service
public class FahuoshangpinService {

    @Resource
    private FahuoshangpinMapper mapper;

    public ResponseData<Fahuoshangpin> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Fahuoshangpin>> selectAll(Wrapper<Fahuoshangpin> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Fahuoshangpin>> selectAll() {
        QueryWrapper<Fahuoshangpin> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Fahuoshangpin> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Fahuoshangpin> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Fahuoshangpin> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("fahuoid"))) {
            wrapper.eq("fahuoid", map.get("fahuoid"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Fahuoshangpin>> filter(Map<String, Object> map) {
        Class<Fahuoshangpin> aClass = Fahuoshangpin.class;
        com.jntoo.db.QueryWrapper<Fahuoshangpin> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Fahuoshangpin> wrapper, IPage page) {
    ...
    public ResponseData insert(Fahuoshangpin entityData, Map post) {
        Info.handlerNullEntity(entityData);

        mapper.insert(entityData);
        if (entityData.getId() != null) {
            Integer charuid = entityData.getId();
            DB.execute("UPDATE fahuoshangpin SET xiaoji=(jiage*shuliang) WHERE id='" + charuid + "'");
    ...
    public ResponseData<Object> update(Fahuoshangpin entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE fahuoshangpin SET xiaoji=(jiage*shuliang) WHERE id='" + post.get("id") + "'");
