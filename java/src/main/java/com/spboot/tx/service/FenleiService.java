...
public class FenleiService {
    ...
    @Resource
    private FenleiMapper mapper;

    public ResponseData<Fenlei> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Fenlei>> selectAll(Wrapper<Fenlei> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Fenlei>> selectAll() {
        QueryWrapper<Fenlei> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Fenlei> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Fenlei> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Fenlei> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("fenleiming"))) {
            wrapper.like("fenleiming", map.get("fenleiming"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Fenlei>> filter(Map<String, Object> map) {
        Class<Fenlei> aClass = Fenlei.class;
        com.jntoo.db.QueryWrapper<Fenlei> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Fenlei> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Fenlei entityData, Map post) {
    ...
    public ResponseData<Object> update(Fenlei entityData, Map post) {
