...
public class GouwucheService {
    ...
    public QueryWrapper<Gouwuche> mapToWrapper(Map<String, Object> map) {
        ...
        String where = " 1=1 ";
        // 判断URL 参数jiajuid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("jiajuid")) && Convert.toInt(map.get("jiajuid")) > 0) {
            // 大于0 则写入条件
    ...
    public ResponseData insert(Gouwuche entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE gouwuche SET xiaoji=(jiage*shuliang) WHERE id='" + charuid + "'");
    ...
    public ResponseData<Object> update(Gouwuche entityData, Map post) {
        ...
        Integer charuid = entityData.getId();
        DB.execute("UPDATE gouwuche SET xiaoji=(jiage*shuliang) WHERE id='" + post.get("id") + "'");
