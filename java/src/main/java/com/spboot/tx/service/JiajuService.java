...
@Service
public class JiajuService {

    @Resource
    private JiajuMapper mapper;

    public ResponseData<Jiaju> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Jiaju>> selectAll(Wrapper<Jiaju> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Jiaju>> selectAll() {
        QueryWrapper<Jiaju> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Jiaju> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Jiaju> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Jiaju> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( mingcheng LIKE '%" + map.get("keyword") + "%' OR pinpai LIKE '%" + map.get("keyword") + "%' OR bianhao LIKE '%" + map.get("keyword") + "%' ) ");
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Jiaju>> filter(Map<String, Object> map) {
        Class<Jiaju> aClass = Jiaju.class;
        com.jntoo.db.QueryWrapper<Jiaju> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Jiaju> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Jiaju entityData, Map post) {
    ...
    public ResponseData<Object> update(Jiaju entityData, Map post) {
