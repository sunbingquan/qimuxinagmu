...
public class ShoucangService {
    ...
    public ResponseData<Shoucang> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Shoucang>> selectAll(Wrapper<Shoucang> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Shoucang>> selectAll() {
        QueryWrapper<Shoucang> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shoucang> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Shoucang> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Shoucang> wrapper = Wrappers.query();
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Shoucang>> filter(Map<String, Object> map) {
        Class<Shoucang> aClass = Shoucang.class;
        com.jntoo.db.QueryWrapper<Shoucang> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Shoucang> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Shoucang entityData, Map post) {
        Info.handlerNullEntity(entityData);
        entityData.setUsername(Request.user().getUsername());
        entityData.setAddtime(Info.getDateStr());

        Map res = DB.name("shoucang").where("biao", post.get("biao")).where("xwid", post.get("xwid")).where("username", Request.user().getUsername()).find();
        if (res != null && !res.isEmpty()) {
            DB.name("shoucang").where("id", res.get("id")).delete();
            return JsonResult.success("已取消收藏");
        }
    ...
    public ResponseData<Object> update(Shoucang entityData, Map post) {
