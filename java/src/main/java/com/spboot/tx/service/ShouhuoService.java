...
public class ShouhuoService {
    ...
    @Resource
    private ShouhuoMapper mapper;

    public ResponseData<Shouhuo> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Shouhuo>> selectAll(Wrapper<Shouhuo> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Shouhuo>> selectAll() {
        QueryWrapper<Shouhuo> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shouhuo> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Shouhuo> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Shouhuo> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( dingdanhao LIKE '%" + map.get("keyword") + "%' ) ");
        }
        // 判断URL 参数fahuoid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("fahuoid")) && Convert.toInt(map.get("fahuoid")) > 0) {
            // 大于0 则写入条件
        ...
        if (!StringUtil.isNullOrEmpty(map.get("kuaididanhao"))) {
            wrapper.like("kuaididanhao", map.get("kuaididanhao"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("kuaidigongsi"))) {
            wrapper.eq("kuaidigongsi", map.get("kuaidigongsi"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Shouhuo>> filter(Map<String, Object> map) {
        Class<Shouhuo> aClass = Shouhuo.class;
        com.jntoo.db.QueryWrapper<Shouhuo> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Shouhuo> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Shouhuo entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("update dingdan set zhuangtai='已收货' where dingdanhao='" + post.get("dingdanhao") + "'");

            DB.execute(
                "INSERT INTO shouhuoshangpin(shouhuoid,jiajuid,bianhao,mingcheng,fenlei,pinpai,tupian,jiage,shuliang,xiaoji,goumairen) SELECT '" +
                charuid +
                "',jiajuid,bianhao,mingcheng,fenlei,pinpai,tupian,jiage,shuliang,xiaoji,goumairen FROM fahuoshangpin WHERE fahuoid='" +
                post.get("fahuoid") +
                "'"
            );
    ...
    public ResponseData<Object> update(Shouhuo entityData, Map post) {
