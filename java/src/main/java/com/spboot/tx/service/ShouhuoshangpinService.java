...
@Service
public class ShouhuoshangpinService {

    @Resource
    private ShouhuoshangpinMapper mapper;

    public ResponseData<Shouhuoshangpin> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Shouhuoshangpin>> selectAll(Wrapper<Shouhuoshangpin> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Shouhuoshangpin>> selectAll() {
        QueryWrapper<Shouhuoshangpin> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shouhuoshangpin> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Shouhuoshangpin> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Shouhuoshangpin> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("shouhuoid"))) {
            wrapper.eq("shouhuoid", map.get("shouhuoid"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("jiajuid"))) {
            wrapper.eq("jiajuid", map.get("jiajuid"));
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("pinpai"))) {
            wrapper.like("pinpai", map.get("pinpai"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("shuliang_start"))) {
            wrapper.ge("shuliang", map.get("shuliang_start"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("shuliang_end"))) {
            wrapper.le("shuliang", map.get("shuliang_end"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("xiaoji_start"))) {
            wrapper.ge("xiaoji", map.get("xiaoji_start"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("xiaoji_end"))) {
            wrapper.le("xiaoji", map.get("xiaoji_end"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Shouhuoshangpin>> filter(Map<String, Object> map) {
        Class<Shouhuoshangpin> aClass = Shouhuoshangpin.class;
        com.jntoo.db.QueryWrapper<Shouhuoshangpin> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Shouhuoshangpin> wrapper, IPage page) {
    ...
    public ResponseData insert(Shouhuoshangpin entityData, Map post) {
        Info.handlerNullEntity(entityData);

        mapper.insert(entityData);
        if (entityData.getId() != null) {
            Integer charuid = entityData.getId();
            DB.execute("UPDATE shouhuoshangpin SET xiaoji=(jiage*shuliang) WHERE id='" + charuid + "'");
    ...
    public ResponseData<Object> update(Shouhuoshangpin entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE shouhuoshangpin SET xiaoji=(jiage*shuliang) WHERE id='" + post.get("id") + "'");
