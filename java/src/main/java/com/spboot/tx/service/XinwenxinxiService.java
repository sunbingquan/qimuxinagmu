...
@Service
public class XinwenxinxiService {

    @Resource
    private XinwenxinxiMapper mapper;

    public ResponseData<Xinwenxinxi> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Xinwenxinxi>> selectAll(Wrapper<Xinwenxinxi> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Xinwenxinxi>> selectAll() {
        QueryWrapper<Xinwenxinxi> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Xinwenxinxi> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Xinwenxinxi> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Xinwenxinxi> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( biaoti LIKE '%" + map.get("keyword") + "%' OR neirong LIKE '%" + map.get("keyword") + "%' ) ");
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("faburen"))) {
            wrapper.like("faburen", map.get("faburen"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Xinwenxinxi>> filter(Map<String, Object> map) {
        Class<Xinwenxinxi> aClass = Xinwenxinxi.class;
        com.jntoo.db.QueryWrapper<Xinwenxinxi> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Xinwenxinxi> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Xinwenxinxi entityData, Map post) {
    ...
    public ResponseData<Object> update(Xinwenxinxi entityData, Map post) {
    ...
    public ResponseData webDetail(Integer id) {
        Map post = DB.name("xinwenxinxi").find(id);
        DB.execute("UPDATE xinwenxinxi SET dianjilv=dianjilv+1 WHERE id=" + post.get("id") + "");
