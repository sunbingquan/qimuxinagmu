        ...
        <el-card class="box-card">
            ...
            <div class="form-search">
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="快递单号">
                        <el-input v-model="search.kuaididanhao"></el-input>
                    </el-form-item>
                    <el-form-item label="快递公司">
                        <el-select v-model="search.kuaidigongsi"
                            ><el-option label="请选择" value=""></el-option><el-option label="顺丰" value="顺丰"></el-option>
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="快递单号" width="130">
                    <template slot-scope="{row}"> {{ row.kuaididanhao }} </template>
                </el-table-column>
                <el-table-column label="快递公司" width="80">
                    <template slot-scope="{row}"> {{ row.kuaidigongsi }} </template>
                </el-table-column>
                <el-table-column label="快递电话" width="130">
                    <template slot-scope="{row}"> {{ row.kuaididianhua }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/fahuodetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit" @click="$goto({path:'/admin/fahuoupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
        ...
        methods: {
            ...
            deleteItems(items) {
                this.$confirm("确定删除数据？此操作不可恢复", "警告", {
                    ...
                    this.$store.dispatch("fahuo/delete", ids).then((res) => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
