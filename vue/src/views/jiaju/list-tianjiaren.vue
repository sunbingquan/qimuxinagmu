        ...
        <el-card class="box-card">
                ...
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="分类">
                        <el-select v-model="search.fenlei"
                            ><el-option label="请选择" value=""></el-option><e-select-option type="option" module="fenlei" value="id" label="fenleiming"></e-select-option
                        ></el-select>
                    </el-form-item>
                    <el-form-item label="品牌">
                        <el-input v-model="search.pinpai"></el-input>
                    </el-form-item>
                    <el-form-item>
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="分类" width="80">
                    <template slot-scope="{row}"> <e-select-view module="fenlei" :value="row.fenlei" select="id" show="fenleiming"></e-select-view> </template>
                </el-table-column>
                <el-table-column label="品牌" width="130">
                    <template slot-scope="{row}"> {{ row.pinpai }} </template>
                </el-table-column>
                ...
                <el-table-column label="销量" width="80">
                    <template slot-scope="{row}"> {{ row.xiaoliang }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/jiajudetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit" @click="$goto({path:'/admin/jiajuupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                filter.session_name = "tianjiaren";

                this.$store
                    .dispatch("jiaju/query", filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItems(items) {
                this.$confirm("确定删除数据？此操作不可恢复", "警告", {
                    ...
                    this.$store.dispatch("jiaju/delete", ids).then((res) => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
